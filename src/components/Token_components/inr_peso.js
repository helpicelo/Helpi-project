...
class INRT_cPESO extends Component {
  ...
  b_a = (bto, bamount) => {
    ...
    this.state.btoken.methods.approve(this.state.mutatetoken._address, bamount).send({ from: this.state.account }).on('transactionHash', (hash) => {
      this.state.mutatetoken.methods.mutate(bTokenaddress, aTokenaddress, bto, bamount, this.state.exchangerate, false).send({ from: this.state.account }).on('transactionHash', (hash) => {
        this.setState({ loading: false})
      })
    })
  }
